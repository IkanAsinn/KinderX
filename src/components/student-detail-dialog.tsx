...
import { Loader2, Trash2, Check, ChevronsUpDown, Upload, X } from "lucide-react"
...
export function StudentDetailDialog({ student, parents }: { student: Student, parents: Parent[] }) {
  ...
  const [photoPreview, setPhotoPreview] = useState<string | null>(student.profile_picture || null)
  ...
  // Reset form when dialog opens so it gets latest data if modified elsewhere
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    // Check if NISN changed and already exists (if not empty)
    if (values.nisn && values.nisn !== student.nisn) {
      ...
      if (existingStudent) {
        toast.error("Gagal update", { description: "Siswa dengan NISN ini sudah terdaftar." })
        setIsLoading(false)
        return
      }
    }

    let profilePictureUrl = photoPreview
    ...
    } else if (!photoPreview && student.profile_picture) {
      // If photoPreview is null but student originally had a picture, it means it was removed
      // Note: we should ideally delete it from storage, but for now we just clear the db column
    ...
    toast.success("Berhasil", { description: "Data siswa telah diperbarui." })
    setOpen(false)
    router.refresh()
    setIsLoading(false)
  }
  ...
  async function handleDelete() {
    if (!confirm("Apakah Anda yakin ingin menghapus siswa ini? Semua data terkait akan ikut terhapus.")) {
      return
    }
    ...
    toast.success("Terhapus", { description: "Data siswa berhasil dihapus." })
    setOpen(false)
    router.refresh()
    setIsDeleting(false)
  }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogTrigger
        render={<Button variant="ghost" size="sm" className="text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 rounded-lg font-medium px-4 h-9" />}
      >
        Detail
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px] max-h-[85vh] overflow-y-auto rounded-3xl border-none shadow-2xl p-8">
        <DialogHeader className="mb-2">
          <DialogTitle className="font-heading text-xl">Detail Siswa</DialogTitle>
          <DialogDescription className="text-slate-500 pt-1">
            Lihat dan perbarui informasi siswa.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5 mt-4">
            ...
            <div className="flex justify-center mb-6">
              <div className="relative group">
                <div 
                  className={cn(
                    "w-24 h-24 rounded-full border-2 border-dashed flex items-center justify-center overflow-hidden transition-colors cursor-pointer",
                    photoPreview ? "border-transparent bg-slate-100" : "border-slate-300 bg-slate-50 hover:border-primary hover:bg-primary/5"
                  )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Kosong..." {...field} disabled={isLoading || isDeleting} className="rounded-xl h-11 px-4" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col" ref={triggerRef}>
                  <FormLabel className="text-slate-700 font-semibold mb-1">Orang Tua / Wali</FormLabel>
                        ...
                        <CommandList>
                          ...
                          <CommandGroup>
                            ...
                            {parents.map((parent) => (
                              <CommandItem
                                key={parent.id}
                                value={parent.full_name} // Command uses this for filtering
                                onSelect={() => {
            ...
            <div className="flex justify-between pt-6 border-t border-slate-100 mt-6">
              <Button 
                ...
                disabled={isLoading || isDeleting} 
                className="text-rose-600 hover:text-rose-700 hover:bg-rose-50 h-11 px-4 rounded-xl"
              >
